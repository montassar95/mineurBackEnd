...
//		String status;
	...
	private static String generateTimeUnitString(int value, String singular, String dual, String plural, String fallback) {
		if (value == 1) {
			return " " + singular + " ";
		} else if (value == 2) {
			return " " + dual + " ";
	...
	public  static String generateLegalCaseString(Affaire affaire) {
		...
		if (affaire.getAffaireAffecter() != null) {
			...
			if (affaire.getTypeDocument().toString().equals("AEX")) {
...
//		 
			...
			System.err.println(affaire.getAffaireId().getIdEnfant()+ " "+ affaire.getAffaireId().getNumAffaire());
			 ...
			 String typeFileAEX = affaire.getTypeFile();
		        String typeAEX = (typeFileAEX == null ||  typeFileAEX.equals("AEX")) ? "إيقاف تنفيذ الحكم" : "ســــــــــــراح";
		         ...
		         addCellulHistorique(p1, c1, tableTypeDocument,   typeAEX);
	...
	public static PdfPCell createPdfPCell(Phrase phrase, int colSpan, int horizontalAlignment, BaseColor backgroundColor) {
	...
	public static void buildAffaireTables(List < Affaire > affaires, PdfPTable tableAffaire) {
	    ...
	    for (int j = 0; j < affaires.size(); j++) {
	      ...
	      p1 = new Phrase(ToolsForReporting.buildTitreTypeAffaire(affaires.get(j)),
	      ...
	      ToolsForReporting.processTableTypeDocument(p1, c1, tableTypeDocument,
	        affaires.get(j));
...
//		System.out.println(residenceDTO.getResidenceId().toString() + " Residence ");
	...
	public static Phrase buildEnfantDetailsPhrase(Enfant enfant) {
	    return new Phrase(enfant.getNom() + " بن " + enfant.getNomPere() + " بن " + enfant.getNomGrandPere() + " " + enfant.getPrenom(), ToolsForReporting.boldfontFamielle);
	}
	
	public static Phrase buildEnfantAddressPhrase(Enfant enfant) {
	    return new Phrase(enfant.getAdresse().toString().trim() + " " + enfant.getDelegation().getLibelle_delegation().toString() +
	            " " + enfant.getGouvernorat().getLibelle_gouvernorat().toString(), ToolsForReporting.boldfontLabelAmiri14);
	}

	public static Phrase buildEnfantBirthDetailsPhrase(Enfant enfant) {
	    return new Phrase("بـــــــ" + enfant.getLieuNaissance() + " " + enfant.getDateNaissance(), ToolsForReporting.boldfontLabelAmiri14);
	}
	...
	public static Phrase buildEnfantSocialDetailsPhrase(Enfant enfant) {
	    if (enfant.getSituationSocial() != null) {
	        ...
	        return new Phrase(enfant.getSituationFamiliale().getLibelle_situation_familiale().toString().trim(), ToolsForReporting.boldfontLabelAmiri14);
	    }
	}
	...
	public static Phrase buildEnfantOccupationDetailsPhrase(Enfant enfant) {
	    if (enfant.getMetier() != null) {
	        ...
	        return new Phrase(" " + enfant.getNiveauEducatif().getLibelle_niveau_educatif().toString().trim(), ToolsForReporting.boldfontLabelAmiri14);
	    }
	}
	...
	private static boolean hasEntryMutationData(Residence residenceDTO) {
	    return residenceDTO.getDateEntree() != null && residenceDTO.getEtablissementEntree() != null && residenceDTO.getCauseMutation() != null;
	}

	private static boolean hasExitMutationData(Residence residenceDTO) {
	    return residenceDTO.getDateSortie() != null && residenceDTO.getEtablissementSortie() != null && residenceDTO.getCauseMutationSortie() != null;
	}
    ...
    public static PdfPTable addTitleToPdfTable( String title, Font font, int colSpan, BaseColor backgroundColor, int border) {
    ...
    private void addTableHeader(PdfPTable table, String headerText) {
        ToolsForReporting.addCellToTable(table, headerText, 100, Element.ALIGN_CENTER, new BaseColor(210, 210, 210), 30);
    }
