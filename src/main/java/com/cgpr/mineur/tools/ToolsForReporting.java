...
import com.cgpr.mineur.config.AffaireDTO;
...
public class ToolsForReporting {
	...
	public static Font boldfontLabel18 = boldConf.getFontForArabic(18);
	...
	public static Font boldfontLabelAmiri11Red = boldConf.getFontForArabicAmiri(12);
	static {
		boldfontFamielle.setColor(BaseColor.BLUE);
		boldfontFamielle11.setColor(new BaseColor(51, 0, 0));

		boldfontLabelAmiri11Red.setColor(BaseColor.RED);
	}
	...
	private static String generateTimeUnitString(int value, String singular, String dual, String plural,
			String fallback) {
		if (value == 1) {
			return " " + singular + " ";
		} else if (value == 2) {
	...
	public static void processEvenementsJuridique(AffaireDTO affaire, Phrase p1,
			PdfPCell c1, PdfPTable tableTypeDocument) {

		try {
			// Ajouter l'événement lui-même
			addCellulHistorique(p1, c1, tableTypeDocument, affaire.getDocumentAssocie().toString());
			if (!affaire.getDocumentAssocie().toString().equals(affaire.getDocumentDernier().toString())) {

				addCellulHistorique(p1, c1, tableTypeDocument, affaire.getDocumentDernier().toString());
			}

			addCellulHistorique(p1, c1, tableTypeDocument, affaire.getDebutPunitionAffaire().toString());
			addCellulHistorique(p1, c1, tableTypeDocument, affaire.getFinPunitionAffaire().toString());

		} catch (Exception e) {
			System.out.println("Erreur lors de l'ajout de l'événement juridique : " + e.getMessage());
		}

	}

	// public static void processTableTypeDocument(Phrase p1, PdfPCell c1, PdfPTable
	// tableTypeDocument, Affaire affaire) {
	//
	// try {
	...
	//
	// addCellulHistorique(p1, c1, tableTypeDocument,
	// affaire.getDateEmissionDocument().toString().trim());
	//
	...
	//
	// addCellulHistorique(p1, c1, tableTypeDocument,
	// affaire.getDateDebutPunition().toString().trim());
	//
	...
	// !affaire.getTypeDocument().toString().equals("AP")) {
	// addCellulHistorique(p1, c1, tableTypeDocument,
	// affaire.getDateFinPunition().toString().trim());
	//
	...
	public static PdfPTable createTopTable(String gouvernorat, String date) throws Exception {
		...
		Phrase p1Top = new Phrase(boldConf.format(date) + " " + boldConf.format(gouvernorat.trim() + " " + "في"),
				boldfontLabelTop);
	...
	public static PdfPTable createTitleTableXX(String titreString) {
		PdfPTable tTitre = new PdfPTable(1);
		...
		cTitre.setPadding(40f);
		// cTitre.setBorder(Rectangle.BOX);

		// cTitre.setBackgroundColor(new BaseColor(240, 240, 240));
		cTitre.setBorderColor(BaseColor.WHITE);
		cTitre.setHorizontalAlignment(Element.ALIGN_CENTER);

		tTitre.setSpacingBefore(20f);
		tTitre.addCell(cTitre);
	...
	public static void buildNumArrestationTables(DetenuStatutDTO rapportDetentionDTO, PdfPTable tableAffaire,
			int i) {
		...
		p1 = new Phrase(rapportDetentionDTO.getNumeroEcrou(), ToolsForReporting.boldfontLabelAmiri14);
		...
		p1 = new Phrase(rapportDetentionDTO.getEtablissementActuel(), ToolsForReporting.boldfontLabelAmiri14);
		...
		p1 = new Phrase(rapportDetentionDTO.getClassePenale(), ToolsForReporting.boldfontLabelAmiri14);

		c1 = ToolsForReporting.createPdfPCell(p1, 100, Element.ALIGN_CENTER, null);
		tableNumArr.addCell(c1);

		p1 = new Phrase(rapportDetentionDTO.getNationalite(), ToolsForReporting.boldfontLabelAmiri14);
		...
		c1.setBorder(0);
		tableNumArr.addCell(c1);
		p1 = new Phrase(rapportDetentionDTO.getAge() + "", ToolsForReporting.boldfontFamielle13);
		...
		p1 = new Phrase(rapportDetentionDTO.getNbrVisite() + " عدد الزيارات", ToolsForReporting.boldfontFamielle12);
		...
		p1 = new Phrase(rapportDetentionDTO.getSituationJudiciaire(), ToolsForReporting.boldfontFamielle);
		...
		// if (rapportDetentionDTO.isEvasion()) {
	...
	public static void buildIdentiteTables(PdfPTable tableAffaire, DetenuStatutDTO rapportDetentionDTO,
			String etatJuridiue, int i) {
		...
		p1 = new Phrase(rapportDetentionDTO.getDetentionId(), ToolsForReporting.boldfontLabelAmiri14);
		...
		p1 = new Phrase(rapportDetentionDTO.getNomComplet(), ToolsForReporting.boldfontFamielle);
		...
		p1 = new Phrase(rapportDetentionDTO.getAdresse(), ToolsForReporting.boldfontLabelAmiri14);
		...
		p1 = new Phrase(rapportDetentionDTO.getDetailsNaissance(), ToolsForReporting.boldfontLabelAmiri14);
		...
		p1 = new Phrase(rapportDetentionDTO.getSituationFamilialeEtSociale(), ToolsForReporting.boldfontLabelAmiri14);
		...
		p1 = new Phrase(rapportDetentionDTO.getNiveauDeFormationEtProfession(), ToolsForReporting.boldfontLabelAmiri14);
		...
		p1 = new Phrase(rapportDetentionDTO.getDateArrestation() + " " + "تــاريخ الإيقـاف ",
		...
		Date dateDebut = rapportDetentionDTO.getDateDebutPeine();
		if (dateDebut != null) {
		...
		Date dateFin = rapportDetentionDTO.getDateFinPeine();
		if (dateFin != null) {

			p1 = new Phrase(simpleDateFormat.format(dateFin) + " " + "  تــاريخ نهاية العقــاب  ",
					ToolsForReporting.boldfontFamielle13);
		...
		if (rapportDetentionDTO.getEvenementEntree() != null && !rapportDetentionDTO.getEvenementEntree().isEmpty()) {

			p1 = new Phrase(rapportDetentionDTO.getEvenementEntree(), ToolsForReporting.boldfontFamielle13);
		...
		// if (!rapportDetentionDTO.getEvenementEntree().isEmpty()) {
		//
		// p1 = new Phrase(rapportDetentionDTO.getMouvementSortie(),
		// ToolsForReporting.boldfontFamielle13);
		//
		// c1 = ToolsForReporting.createPdfPCell(p1, 100, Element.ALIGN_CENTER,
		// BaseColor.WHITE);
		// c1.setBorder(0);
		//
		// tableIdentite.addCell(c1);
		// }
		// if (rapportDetentionDTO.isMutationEnCours()) {
		//
		...
		// if (rapportDetentionDTO.getLiberatio.isEmpty()) {
		...
		// }
		//
		// if (!rapportDetentionDTO.getLiberation().isEmpty()) {
		// p1 = new Phrase(rapportDetentionDTO.getLiberation(),
		// ToolsForReporting.boldfontFamielle13);
		//
		// c1 = ToolsForReporting.createPdfPCell(p1, 100, Element.ALIGN_CENTER,
		// BaseColor.WHITE);
		...
		if (etatJuridiue != null && etatJuridiue.toString().equals("seraLibere")) {
			...
			p1 = new Phrase(rapportDetentionDTO.getDateFinPeine().toString() + " " + "يفرج عنه بتاريخ",
					ToolsForReporting.boldfontFamielle13);
			c1 = ToolsForReporting.createPdfPCell(p1, 100, Element.ALIGN_CENTER, BaseColor.WHITE);
		...
		buildNumArrestationTables(rapportDetentionDTO, tableAffaire, i);
	...
	public static void addCellToTable(PdfPTable table, String text, int colspan, int horizontalAlignment,
			BaseColor backgroundColor, int fixedHeight) {
	...
	public static void addCellVideToTable(PdfPTable table, String text, int colspan, int horizontalAlignment,
			BaseColor backgroundColor, int fixedHeight) {
	...
	public static void addCellToTableforStat(PdfPTable table, String text, int colspan, int horizontalAlignment,
			BaseColor backgroundColor, int fixedHeight) {

		Phrase phrase = new Phrase(text, ToolsForReporting.boldfontLabel18);
		PdfPCell cell = new PdfPCell(phrase);
		cell.setRunDirection(PdfWriter.RUN_DIRECTION_LTR);
		cell.setHorizontalAlignment(Element.ALIGN_CENTER);
		cell.setColspan(colspan);
	...
	public static void buildAffaireTables(List<AffaireDTO> affaires,
			PdfPTable tableAffaire) {
		...
		for (int j = 0; j < affaires.size(); j++) {
			...
			p1 = new Phrase(affaires.get(j).getTypeAffaire(), ToolsForReporting.boldfontLabelAmiri13);
			...
			p1 = new Phrase(affaires.get(j).getTribunal(), ToolsForReporting.boldfontFamielle13);
			...
			p1 = new Phrase(affaires.get(j).getNumeroAffaire(), ToolsForReporting.boldfontLabelAmiri13);
			...
			if (affaires.get(j).getAccusations() == null) {
				p1 = new Phrase("تهمة غير متوفرة ", ToolsForReporting.boldfontLabelAmiri11Red);

			} else {
				p1 = new Phrase(affaires.get(j).getAccusations().trim(), ToolsForReporting.boldfontLabelAmiri11);
			}
			...
			p1 = new Phrase(affaires.get(j).getPunition(), ToolsForReporting.boldfontLabelAmiri14);
			c1 = ToolsForReporting.createPdfPCell(p1, 100, Element.ALIGN_CENTER, new BaseColor(245, 245, 245));
			...
			ToolsForReporting.processEvenementsJuridique(affaires.get(j), p1, c1,
					tableTypeDocument);
	...
	public static void processTablePrencipal(DetenuStatutDTO rapportDetentionDTO, PdfPTable tableAffaire,
			String etatJuridiue, int i) {

		if (rapportDetentionDTO != null) {

			List<AffaireDTO> affaires = rapportDetentionDTO.getAffaires();
			...
			buildIdentiteTables(tableAffaire, rapportDetentionDTO, etatJuridiue, i);

		}
	}

	// public static Phrase buildEnfantDetailsPhrase(String enfant) {
	// return new Phrase(enfant, ToolsForReporting.boldfontFamielle);
	// }
	//
	// public static Phrase buildEnfantAddressPhrase(String enfant) {
	// return new Phrase(enfant, ToolsForReporting.boldfontLabelAmiri14);
	// }
	//
	// public static Phrase buildEnfantBirthDetailsPhrase(String enfant) {
	// return new Phrase(enfant, ToolsForReporting.boldfontLabelAmiri14);
	// }

	// public static Phrase buildEnfantSocialDetailsPhrase(String enfant) {
	//
	// return new Phrase(enfant, ToolsForReporting.boldfontLabelAmiri14);
	//
	// }

	// public static Phrase buildEnfantOccupationDetailsPhrase(String enfant) {
	//
	// return new Phrase(enfant, ToolsForReporting.boldfontLabelAmiri14);
	//
	// }

	// public static Phrase buildEntryMutationPhrase(String residenceDTO) {
	// return new Phrase(residenceDTO, ToolsForReporting.boldfontFamielle13);
	// }

	// public static Phrase buildExitMutationPhrase(String residenceDTO) {
	// return new Phrase(residenceDTO, ToolsForReporting.boldfontFamielle13);
	// }
