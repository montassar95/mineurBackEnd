...
//     List<ArretProvisoire>  getArretProvisoirebyArrestation  (String idEnfant,long numOrdinalArrestation );
	...
	@Query("SELECT a  FROM ArretProvisoire a WHERE a.carteRecup.documentId.idEnfant = ?1  and"
			+ " a.carteRecup.documentId.numOrdinalArrestation = ?2  order by a.dateDebut" )
     List<ArretProvisoire>  getArretProvisoirebyArrestation  (String idEnfant,long numOrdinalArrestation );	
