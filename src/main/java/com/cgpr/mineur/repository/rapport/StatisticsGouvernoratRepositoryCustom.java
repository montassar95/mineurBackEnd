...
@Repository
public class StatisticsGouvernoratRepositoryCustom {

    @PersistenceContext
    private EntityManager entityManager;

    public List<StatistiqueGouvernoratDTO> findStatistics( LocalDate dateFin) throws IOException {
        // Charger la requête SQL depuis le fichier
        String sql = SQLLoader.loadSQL("sql/get_statistics_gouvernorat.sql");
        ...
        List<StatistiqueGouvernoratDTO> statistiques = new ArrayList<>();

        // Mapper les résultats dans des objets DTO
        for (Object[] row : results) {
            String gouvernorat     = (String) row[1];
            Long arretemasculin     = ((Number) row[2]).longValue();
            Long  arretefeminin  = ((Number) row[3]).longValue();
            Long jugemasculin = ((Number) row[4]).longValue();
            Long jugefeminin = ((Number) row[5]).longValue();
            ...
            StatistiqueGouvernoratDTO dto = new StatistiqueGouvernoratDTO(
            		gouvernorat, 
            		
            		arretemasculin, arretefeminin, 
            		jugemasculin , jugefeminin
            );
