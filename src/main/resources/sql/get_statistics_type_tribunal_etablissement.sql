...
last_document AS (
    ...
    GROUP BY doc.id_enf, doc.num_ord_arr, doc.num_ord_aff  , cte.ETAFK   
),
...
last_document_by_aff AS (
    ...
    LEFT JOIN doc doc 
        ...
        AND TRUNC(doc.date_emission) <=  :dateFin  
    GROUP BY ld.id_enf, ld.num_ord_arr, ld.num_ord_aff ,ld.max_num_ord_doc ,ld.ETAFK , doc.date_emission 
) 
  ...
  CEIL(MONTHS_BETWEEN(:dateFin, TRUNC(ldba.date_emission))) AS mois_resides,
    ...
    GROUP BY cte.id_enf, cte.ETAFK ,  ldba.date_emission, etablissement.LIBELLE_ETABLISSEMENT,    tribunal.TYP_TRIFK,  type_tribunale.LIBELLE_TYPE_TRIBUNAL , cte.statut ORDER BY mois_resides desc  ) 
...
SELECT  
    ...
    SUM(CASE WHEN ETAFK = '11' THEN 1 ELSE 0 END) AS mourouj,
    SUM(CASE WHEN ETAFK = '12' THEN 1 ELSE 0 END) AS sidiheni,
    SUM(CASE WHEN ETAFK = '13' THEN 1 ELSE 0 END) AS soukjdid,
    SUM(CASE WHEN ETAFK = '14' THEN 1 ELSE 0 END) AS mghira,
    SUM(CASE WHEN ETAFK = '16' THEN 1 ELSE 0 END) AS mjazbeb,

    -- Total par ligne
    SUM(
        CASE WHEN ETAFK IN ('11', '12', '13', '14', '16') THEN 1 ELSE 0 END
    ) AS total,
...
SELECT  
    ...
    SUM(CASE WHEN ETAFK = '11' THEN 1 ELSE 0 END),
    SUM(CASE WHEN ETAFK = '12' THEN 1 ELSE 0 END),
    SUM(CASE WHEN ETAFK = '13' THEN 1 ELSE 0 END),
    SUM(CASE WHEN ETAFK = '14' THEN 1 ELSE 0 END),
    SUM(CASE WHEN ETAFK = '16' THEN 1 ELSE 0 END),
    ...
    1 AS tri -- pour forcer la dernière ligne
FROM ranked
WHERE rn = 1 AND situation_judiciaire_id = 'arrete'
