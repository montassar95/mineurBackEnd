...
WITH cte_statut AS (
   SELECT 
        ...
        res.num_arrestation, 
        res.ETAFK, 
        -- Ajout de la date d'entrée de la première résidence
    (
        SELECT r1.date_entree
        FROM res r1
        WHERE r1.id_enf = arr.id_enf
          AND r1.NUM_ORD_ARR = arr.num_ord
          AND r1.num_ord_res = 1
        FETCH FIRST 1 ROWS ONLY
    ) AS date_entree_premiere_residence
          ,'Residant'  AS statut    
    FROM arr arr
    LEFT JOIN lib lib 
    ...
    LEFT JOIN (
        ...
        GROUP BY res.id_enf, res.NUM_ORD_ARR , res.ETAFK
    ) res_id 
    INNER JOIN res res 
...
last_document AS (
    SELECT 
        ...
        doc.num_ord_arr, 
        doc.num_ord_aff, 
        cte.date_entree_premiere_residence,
       cte. ETAFK,
    ...
    GROUP BY doc.id_enf, doc.num_ord_arr, doc.num_ord_aff, cte. ETAFK, cte.date_entree_premiere_residence    
),
...
last_document_by_aff AS (
    SELECT 
        ...
        ld. ETAFK,
          doc.date_emission,
          ld.date_entree_premiere_residence,
    ...
    LEFT JOIN doc doc 
        ...
        AND TRUNC(doc.date_emission) <=  :dateFin    
    GROUP BY ld.id_enf, ld.num_ord_arr, ld.num_ord_aff ,  doc.date_emission, ld.max_num_ord_doc,  ld. ETAFK, ld.date_entree_premiere_residence
)
  , alll as(  SELECT 
     cte.id_enf,  
...
CEIL(MONTHS_BETWEEN(:dateFin  , TRUNC(ldba.date_entree_premiere_residence))) AS mois_resides,
 cte. ETAFK,
   etablissement.LIBELLE_ETABLISSEMENT,
        CASE
    ...
    LEFT JOIN gou gouvernement 
      ...
      ON etablissement.id = cte.ETAFK
      
    GROUP BY cte.id_enf,ldba.date_entree_premiere_residence,    tribunal.TYP_TRIFK,  type_tribunale.LIBELLE_TYPE_TRIBUNAL , cte. ETAFK,  etablissement.LIBELLE_ETABLISSEMENT,  cte.statut ORDER BY mois_resides desc  ) 
...
ETAFK,
    ...
    SUM(CASE WHEN mois_resides > 12 AND mois_resides <= 15 THEN 1 ELSE 0 END) AS entre_12_15_mois,
  SUM(CASE WHEN mois_resides > 15 AND mois_resides <= 18 THEN 1 ELSE 0 END) AS entre_15_18_mois,
   SUM(CASE WHEN mois_resides > 18  THEN 1 ELSE 0 END) AS plus_18_mois,
...
GROUP BY LIBELLE_TYPE_TRIBUNAL , ETAFK , LIBELLE_ETABLISSEMENT 
 
UNION ALL

SELECT  
    '  ' AS LIBELLE_TYPE_TRIBUNAL,
    '  ' AS LIBELLE_TYPE_TRIBUNAL,
     'المجموع' AS ETAFK,
    ...
    SUM(CASE WHEN mois_resides > 12 AND mois_resides <= 15 THEN 1 ELSE 0 END)  ,
    SUM(CASE WHEN mois_resides > 15 AND mois_resides <= 18 THEN 1 ELSE 0 END) ,
   SUM(CASE WHEN mois_resides > 18  THEN 1 ELSE 0 END) ,
...
ORDER BY tri, ETAFK ,LIBELLE_TYPE_TRIBUNAL 