...
WITH cte_statut AS (
    SELECT 
        ...
        arr.date_arrestation, 
         CASE
        WHEN lib.date_liberation <= :dateFin THEN lib.date_liberation   -- Si "libre", afficher la date de libération
        ELSE NULL  -- Si "Residant",  afficher NULL
    END AS date_liberation,
     CASE
        WHEN lib.date_liberation <= :dateFin THEN cau_lib.id   -- Si "libre", afficher la date de libération
        ELSE NULL  -- Si "Residant",  afficher NULL
    END AS    cause_liberation_id,
     
   CASE
        WHEN lib.date_liberation <= :dateFin THEN cau_lib.LIBELLE_CAUSE_LIBERATION   -- Si "libre", afficher la date de libération
        ELSE NULL  -- Si "Residant",  afficher NULL
    END AS     cause_liberation,
    -- Statut de detenu
    CASE
        WHEN lib.date_liberation <= :dateFin THEN 'libre'
        ELSE 'Residant'
    END AS statut
    ...
    LEFT JOIN (
        ...
        FROM res
        WHERE res.date_entree <= :dateFin 
        AND (res.date_sortie >= :dateDebut OR res.date_sortie IS NULL)
    ...
    LEFT JOIN CAU_LIB cau_lib 
      ...
      WHERE arr.date_arrestation <= :dateFin 
      AND (lib.date_liberation >= :dateDebut OR lib.date_liberation IS NULL)
...
document_decision AS (
    SELECT 
        ...
        cte.date_liberation,
        cte.cause_liberation_id,
        cte.cause_liberation,
        cte.statut,
        ...
        CASE
            WHEN cte.statut = 'libre' THEN cte.cause_liberation || ' ' ||  TO_CHAR(cte.date_liberation, 'DD-MM-YYYY') 
            WHEN LISTAGG(doc.type_document, ', ') WITHIN GROUP (ORDER BY doc.num_ord_doc) LIKE '%CD%' OR
                 LISTAGG(doc.type_document, ', ') WITHIN GROUP (ORDER BY doc.num_ord_doc) LIKE '%CH%' OR
                 LISTAGG(doc.type_document, ', ') WITHIN GROUP (ORDER BY doc.num_ord_doc) LIKE '%AE%' OR
                 LISTAGG(doc.type_document, ', ') WITHIN GROUP (ORDER BY doc.num_ord_doc) LIKE '%AP%' OR
    ...
    GROUP BY cte.id_enf, cte.num_ord, cte.statut, cte.num_ord_res ,date_arrestation ,date_liberation , cause_liberation_id , cause_liberation
),
...
info_detenu AS (
    SELECT 
        ...
        document_decision.date_liberation,
        document_decision.cause_liberation_id
...
MIN(
    ...
    CASE 
        ...
        WHEN dernier_document.type_document = 'CHL' THEN 'تغير مكان الإيداع'  
    ...
    CASE 
        ...
        WHEN document_associe.type_document = 'CHL' THEN 'تغير مكان الإيداع'
        ELSE 'نوع مستند غير معروف'
...
SELECT    
  ...
  situation_judiciaire  ,
      count(*)
    FROM synthese     where type_affaire = 'إرهاب' group by  etablissement_reside,
          nationalite,
         type_affaire,
  situation_judiciaire
