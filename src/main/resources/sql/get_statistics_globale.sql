...
last_document_by_aff AS (
  ...
  etablissement_situation_judiciaire AS (
    SELECT 
        cte.id_enf, 
        cte.ETAFK as etablissement_id,
        etablissement.libelle_etablissement,
        CASE
            WHEN cte.num_ord = 1 THEN 'primaire'
            ELSE 'revenant'
        END AS classement_penale,
        CASE
    ...
    GROUP BY cte.id_enf, cte.ETAFK, cte.statut, libelle_etablissement, sexe, 
             CASE WHEN cte.num_ord = 1 THEN 'primaire' ELSE 'revenant' END
)
...
GROUP BY  libelle_etablissement ,sexe, classement_penale, situation_judiciaire_id
ORDER BY  libelle_etablissement , situation_judiciaire_id ,classement_penale, sexe 