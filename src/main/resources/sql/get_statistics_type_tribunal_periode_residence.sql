...
last_document AS (
    ...
    GROUP BY doc.id_enf, doc.num_ord_arr, doc.num_ord_aff, cte.date_entree_premiere_residence    
),
...
last_document_by_aff AS (
    ...
    LEFT JOIN doc doc 
        ...
        AND TRUNC(doc.date_emission) <=  :dateFin    
    GROUP BY ld.id_enf, ld.num_ord_arr, ld.num_ord_aff ,  doc.date_emission, ld.max_num_ord_doc, ld.date_entree_premiere_residence
)
  , alll as(  SELECT 
     cte.id_enf,  
...
CEIL(MONTHS_BETWEEN(:dateFin  , TRUNC(ldba.date_entree_premiere_residence))) AS mois_resides,
    ...
    GROUP BY cte.id_enf,ldba.date_entree_premiere_residence,    tribunal.TYP_TRIFK,  type_tribunale.LIBELLE_TYPE_TRIBUNAL , cte.statut ORDER BY mois_resides desc  ) 
