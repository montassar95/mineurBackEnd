...
last_document_by_aff AS (
  ...
  , alll as(  SELECT 
    ...
    GROUP BY cte.id_enf,     tribunal.GOUFK, gouvernement.LIBELLE_GOUVERNORAT , tribunal.NOM_TRIBUNAL , cte.statut ) 
...
SELECT  
    ...
    LIBELLE_GOUVERNORAT,
    NOM_TRIBUNAL,
    SUM(CASE WHEN situation_judiciaire_id = 'arrete' THEN 1 ELSE 0 END) AS arrete,
    SUM(CASE WHEN situation_judiciaire_id = 'juge' THEN 1 ELSE 0 END) AS juge,
    SUM(CASE 
...
FROM ranked
WHERE rn = 1 
GROUP BY GOUFK, LIBELLE_GOUVERNORAT ,NOM_TRIBUNAL
...
SELECT  
    NULL AS GOUFK,
    NULL AS gouvernorat,
    'المجموع' AS NOM_TRIBUNAL,
    SUM(CASE WHEN situation_judiciaire_id = 'arrete' THEN 1 ELSE 0 END),
    SUM(CASE WHEN situation_judiciaire_id = 'juge' THEN 1 ELSE 0 END),
    SUM(CASE 
            WHEN situation_judiciaire_id NOT IN ('arrete', 'juge') OR situation_judiciaire_id IS NULL 
            THEN 1 
            ELSE 0 
...
ORDER BY    GOUFK NULLS LAST,  arrete desc , juge desc